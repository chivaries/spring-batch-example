package com.example.demo.schedule2;

import com.example.demo.schedule2.SpringBatchJobExecutor;
...
public class BatchHelper {
    ...
    /**
     * quartz JobDataMap 로부터 Spring Batch JobParameters 를 추출
     * 이 때 JobDataMap 은 JobDetail 의 JobDataMap 과 Trigger 의 JobDataMap 이 합쳐진 JobDataMap.
     *
     * Spring Batch Job 은 Job Name 과 Job Parameter 로 동일 Job 을 확인하므로,
    ...
    public static class JobDetailFactoryBeanBuilder {
        ...
        /**
         * Spring batch job name 을 map 에 등록
         ...
         */
        public JobDetailFactoryBeanBuilder requestRecovery(boolean requestsRecovery) {
        ...
        public JobDetailFactoryBean build() {
            ...
            // Quartz Job 설정 (Spring batch job 의 어댑터)
            jobDetailFactory.setJobClass(SpringBatchJobExecutor.class);
            jobDetailFactory.setDurability(this.durability);
            jobDetailFactory.setRequestsRecovery(this.requestsRecovery);
            // JobDetail 의 jobDataMap 에 저장
            // SpringBatchJobExecutor 에서 JobExecutionContext.getMergedJobDataMap() 로 접근
